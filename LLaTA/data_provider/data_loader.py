...
class Dataset_ETT_hour(Dataset):
    ...
    def __getitem__(self, index):
        if 'decp_single_linr' in self.model_id:
            s_begin = index
            ...
            for i in range(self.channel):
                ...
                # print(i , component.shape)
                components.append(component)
            components = torch.stack(components) #(7 , 3, seq_len)
...
class Dataset_ETT_minute(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 target='OT', scale=True, timeenc=0, freq='t',  model_id='' , seasonal_patterns=None, percent=100):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None, percent=100 , model_id=''):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        #     print(self.data_x.shape , 'data initialing...')
        #     # (7  , 18412 )
        #     self.data_x = torch.tensor(self.data_x, dtype=torch.float).transpose(1, 0) 
        #     components= []
        #     for i in range(self.data_x.size(0)):
        #         (trend, seasonal, residual) = decompose(self.data_x[i:i+1] , period=self.period)
        #         component = torch.concat([trend, seasonal, residual], dim=0)  # [3, seq_len]
        #         # print(i , component.shape)
        #         components.append(component)
        #     components = torch.stack(components) #(7 , 3, 18412)
        #     self.data_x = components.transpose(2, 0, 1)
        #     #  #(seq_len  , 7 , 3, )
    ...
    def __getitem__(self, index):
        if 'decp_single_linr' in self.model_id:
            ...
            x = torch.tensor(seq_x, dtype=torch.float).transpose(1, 0)  # [7, 3, seq_len]
            y = torch.tensor(seq_y, dtype=torch.float).transpose(1, 0)  # [7, pred_len]
            components= []
