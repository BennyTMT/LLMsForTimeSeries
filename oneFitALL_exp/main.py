...
'''
    ...
    bash ./scripts/ETTh1_datasize.sh
    bash ./scripts/ETTh1_DLinear.sh
    bash ./scripts/ETTh1_attn.sh
    bash ./scripts/ETTh1_noise_wpe.sh
    
    bash ./scripts/all_data_attn.sh
    bash ./scripts/other_data_attn.sh
    
    bash ./scripts/ETT_randomInit.sh
    
''' 

# log_fine_name = 'disturb_TS_0p1.txt'
# log_fine_name = 'origRepo.txt'
# shuffle_type = ['sf_all' ,  'sf_half' ,'ex-half'][gpu_loc]
...
if args.save_file_name is not None : 
    ...
    log_fine_name = "DLinear_{}_{}_trainRatio.txt".format(args.seq_len , args.pred_len)
    with open(log_fine_name , 'a') as f : 
        f.write('training Ratio : {}\n'.format(args.train_ratio))
        
device_address = 'cuda:'+str(args.gpu_loc)

# ratio_name = str(args.train_ratio).replace('.' ,'p')
# log_fine_name = "{}_96_96.txt".format(ratio_name)
...
for ii in range(args.itr):
    ...
    path = './checkpoints/' +  args.model_id
