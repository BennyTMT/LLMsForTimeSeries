...
class GPT4TS(nn.Module):
    '''
        Namespace(model_id='ETTh1_GPT4TS__336_96_100', 
        checkpoints='./checkpoints/', 
        root_path='./datasets/ETT-small/', 
        data_path='ETTh1.csv', data='ett_h', features='M', freq='h', target='OT', embed='timeF', percent=100, 
        seq_len=336, pred_len=96, label_len=168, decay_fac=0.5, learning_rate=0.0001, batch_size=256, 
        num_workers=10, train_epochs=10, lradj='type4', patience=3, gpt_layers=6, is_gpt=1, e_layers=3, 
        d_model=768, n_heads=4, d_ff=768, dropout=0.3, enc_in=7, c_out=7, patch_size=16, kernel_size=25, 
        loss_func='mse', pretrain=1, freeze=1, model='GPT4TS', stride=8, max_len=-1, hid_dim=16, tmax=20, 
        itr=3, cos=1)
     '''
    def __init__(self, configs, device , log_fine_name = None ):
        ...
        if 'Attn_to_Linear' in configs.model_id:
            ...
            self.attn_to_Linear.to(device=device)
            self.attn_to_Linear.train() 
            
        if  'Attn_to_Attn'  in configs.model_id :
